  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-background">
      ...
      <aside className="hidden md:flex md:w-64 bg-sidebar border-r border-sidebar-border shadow-sm flex-col">
        ...
        <div className="px-4 py-4 border-t border-sidebar-border space-y-3">
          <div className="px-3 py-2 bg-sidebar-accent/20 rounded-lg">
            ...
            <p className="text-sm font-semibold text-sidebar-foreground truncate">{user?.name || user?.email}</p>
            <p className="text-xs text-sidebar-foreground/60 uppercase tracking-wide">{user?.role || 'user'}</p>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <nav className="md:hidden px-4 py-4 space-y-2 bg-sidebar border-b border-sidebar-border overflow-y-auto max-h-96">
            {navItems.map((item: any) => {
              const Icon = item.icon;
              const isActive = activePage === item.id;
              return (
